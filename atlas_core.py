...
from atlas_do.constants import (
    _GUIDE_ADVANCE_RE,
    _GUIDE_VERIFY_CONF_LOW,
    _GUIDE_VERIFY_CONF_PASS,
    _TASK_ACTION_MAX_RETRIES,
    _TASK_MAX_STEPS,
    _TASK_PHYSICAL_ACTIONS,
    _TASK_VERIFY_ACTIONS,
)
...
class StateEngine:
    ...
    def set_user_pref(self, key: str, value) -> None:
        ...
        if key == "safety_mode":
            ...
            self._sync_fs_policy()
        if key == "glass_screen_watch":
            self._sync_interview_watcher()

    def get_glass_interview_brief(self) -> str:
        return str(self.get_user_prefs().get("glass_interview_brief") or "").strip()

    def get_glass_auto_answer(self) -> bool:
        pref = self.get_user_prefs().get("glass_auto_answer")
        return True if pref is None else bool(pref)

    def get_glass_screen_watch(self) -> bool:
        pref = self.get_user_prefs().get("glass_screen_watch")
        return True if pref is None else bool(pref)

    def _glass_should_auto_answer(self, question: str, *, cooldown_s: float = 45.0) -> bool:
        from atlas_glass.interview import question_fingerprint

        fp = question_fingerprint(question)
        now = time.time()
        last = self._glass_answer_cooldown.get(fp, 0.0)
        if now - last < cooldown_s:
            return False
        self._glass_answer_cooldown[fp] = now
        return True

    def _sync_interview_watcher(self) -> None:
        watcher = getattr(self, "_interview_watcher", None)
        if watcher is None:
            return
        if (
            self.glass.active
            and getattr(self, "_copilot_active", False)
            and self.get_glass_screen_watch()
        ):
            watcher.start()
        else:
            watcher.stop()
    ...
    def _on_killswitch(self) -> None:
        self.cancel_current()
        self.stop_task()
        if getattr(self, "goals", None):
            self.goals.kill()
        try:
            voice_engine.skip()
        except Exception:
            pass
        self._emit("killswitch", {"active": True})
    ...
    def route_command(self, text: str) -> dict:
        ...
        if tl.startswith("/goal "):
            return {"intent": "start_goal", "goal": t[6:].strip()}
        if tl in ("/goal", "/goal resume"):
            return {"intent": "resume_goal"}
        if tl in ("/goal kill", "/goal stop", "/kill"):
            return {"intent": "kill_goal"}
        if tl in ("/recap", "/weekly", "/weekly-recap") or t.lower() in (
            "weekly recap",
            "summarize my week",
            "what did atlas do this week",
        ):
            return {"intent": "weekly_recap"}
        ...
        if t in ("stop", "cancel", "halt", "stop it", "stop that", "quiet", "enough"):
            return {"intent": "stop"}
        goal_m = re.match(r"^(?:start |run )?goal[:\s]+(.+)$", t)
        if goal_m and len(goal_m.group(1).strip()) > 2:
            return {"intent": "start_goal", "goal": goal_m.group(1).strip()}
    ...
    def execute_command(self, intent: dict) -> bool:
        ...
        try:
            ...
            if kind == "stop":
                ...
                return True
            if kind == "start_goal":
                goal_text = (intent.get("goal") or "").strip()
                if not goal_text:
                    self._announce("Tell me what goal to work on.")
                    return True
                goal_id = self.goals.start(goal_text)
                self._announce(f"Goal started — step 1 of your plan.")
                self._emit("goal_started", {"goal_id": goal_id, "goal": goal_text})
                return True
            if kind == "resume_goal":
                if self.goals.resume_if_needed():
                    self._announce("Resuming your active goal.")
                else:
                    self._announce("No active goal to resume.")
                return True
            if kind == "kill_goal":
                self.engage_killswitch()
                self._announce("Killswitch engaged — all autonomous actions stopped.")
                return True
            if kind == "run_routine":
            ...
            if kind == "weekly_recap":
                ...
                self._emit("weekly_recap", {"text": recap})
                self._announce(recap)
                return True
            if kind == "fs_read":
    ...
    def __init__(
        ...
        self.glass = GlassSession(self.memory, self.user_id)
        ...
        self._glass_answer_cooldown: dict[str, float] = {}
        self._interview_watcher = InterviewScreenWatcher(
            on_question=lambda q: self.handle_glass_interview_question(
                q, source="screen_detect",
            ),
        )
        self._interview_watcher.bind_screen_watcher(self.screen_watcher)
        ...
        self.killswitch.register(self._on_killswitch)
        self.goals = GoalEngine(self)
        self._apply_saved_focus_mode()
        self.goals.resume_if_needed()
    ...
    def set_focus_mode(self, enabled: bool) -> None:
        with self._lock:
            ...
            if self.focus_mode:
                ...
                ctx = fetch_pre_meeting_context(self)
                self.glass.start(title=str(ctx.get("title") or "Focus session"))
                brief = str(ctx.get("brief") or "").strip()
                if brief:
                    self.glass.set_pre_meeting_brief(brief)
                    self.memory.audit_log(
                        self.user_id,
                        "glass",
                        "Pre-meeting brief loaded",
                        {"event": ctx.get("event_title", "")},
                    )
                    self._emit("pre_meeting_brief", {
                        "text": brief,
                ...
                if self.glass.active:
                    ...
                    if result.get("summary"):
                        self._emit("glass_session_ended", result)
                self.set_copilot_mode(False)
                if self.mode == ModeState.INTERVIEW:
                    self.mode = ModeState.ACTIVE
            if self.session.is_active:
                self.session.start(self.get_system_prompt())
            self.set_user_pref("focus_mode", self.focus_mode)
            self._sync_interview_watcher()
            self._emit("glass_changed", {"active": self.focus_mode, **self.glass.status()})
    ...
    def _apply_saved_focus_mode(self) -> None:
        """Restore Glass/Focus session after full engine init (screen watcher ready)."""
        if self.focus_mode:
            self.mode = ModeState.INTERVIEW
    ...
    def handle_input(
        ...
        try:
            ...
            from atlas_mind.router import try_route_tools

            stack_answer = try_stack_answer(text)
            if stack_answer:
                self._finish_router_response(text, stack_answer, tool="stack")
                return

            if not self.glass.active and try_route_tools(self, text):
                return
            ...
            if self.glass.active:
                from atlas_glass.interview import interview_answer_prefix

                enriched_input = f"{interview_answer_prefix(source)}\n{enriched_input}"
            if context_snap:
                enriched_input = (
            ...
            if self.glass.active:
                from atlas_glass.interview import build_interview_brief_block

                speaker_ctx = self.audio_watcher.get_audio_context(30.0)
                glass_block = self.glass.build_context_block(
                    text,
                ...
                if glass_block:
                    memory_prompt = (
                        (memory_prompt + "\n\n" + glass_block).strip()
                        if memory_prompt else glass_block
                    )
                brief_block = build_interview_brief_block(self.get_glass_interview_brief())
                if brief_block:
                    memory_prompt = (
                        (memory_prompt + "\n\n" + brief_block).strip()
                        if memory_prompt else brief_block
                    )
            ...
            if skill_name:
                ...
                inject = str(result.get("inject_context") or "").strip()
                if inject:
                    skill_context = (
                        (skill_context + "\n\n" + inject).strip()
                        if skill_context else inject
                    )
    ...
    def _finish_router_response(
        ...
        *,
        ...
        """Complete a stack/router turn without full LLM streaming."""
        try:
            self.session.push_user(raw_user_text)
            self.session.push_assistant(response)
            self._emit("router_handled", {"tool": tool, "text": response})
            self._on_complete(response)
            self._schedule_learning_on_turn_complete(raw_user_text, response)
            self.memory.record_takeaway_from_turn(
                self.user_id,
    ...
    def _on_ai_query(self, messages: list[dict], raw_user_text: str, had_screen: bool = False) -> None:
        ...
        try:
            from atlas_mind.provider_router import get_provider_router
            ...
            stream_iter = router.stream_chat(
                messages=messages,
                model=model,
                vision=use_vision,
                engine=self,
            ...
            for delta in stream_iter:
                if not provider_announced:
                    if router.last_provider != "groq":
                        self._emit("provider_fallback", {"provider": router.last_provider})
    ...
    def ingest_glass_transcript(self, text: str, source: str) -> None:
        """Record mic/speaker/user text into the active Glass meeting."""
        if self.glass.active:
            self.glass.ingest(text, source)
        src = (source or "").lower()
        if (
            src == "speaker"
            and self.glass.active
            and self.get_glass_auto_answer()
        ):
            from atlas_glass.interview import looks_like_interview_question

            body = (text or "").strip()
            if (
                body
                and looks_like_interview_question(body)
                and self._glass_should_auto_answer(body)
            ):
                threading.Thread(
                    target=self.handle_glass_interview_question,
                    args=(body,),
                    kwargs={"source": "speaker"},
                    daemon=True,
                    name="atlas-glass-speaker",
                ).start()

    def handle_glass_interview_question(
        self,
        question: str,
        *,
        source: str = "screen_detect",
    ) -> None:
        """Fast interview path: screen + Glass context + user brief → answer."""
        question = (question or "").strip()
        if not question or not self.glass.active:
            return
        acquired = self._query_semaphore.acquire(blocking=False)
        if not acquired:
            self._emit("query_rejected", {"source": source, "reason": "busy"})
        ...
        try:
            if self.glass.active:
                self.glass.ingest(question, source)
            self._emit("glass_question_detected", {"question": question, "source": source})
            if not self.session.is_active:
                self.session.start(self.get_system_prompt())
            ...
            enriched = f"{interview_answer_prefix(source)}\n{question}"
            speaker_ctx = self.audio_watcher.get_speaker_context(30.0)
            screen_b64 = None
            try:
                frame = capture_screen_b64()
                if frame:
                ...
                log.debug("glass interview screen capture failed: %s", exc)
            ...
            threading.Thread(
                target=self._on_ai_query,
                args=(messages, question, bool(screen_b64)),
                daemon=True,
                name="atlas-glass-interview",
            ).start()
        except Exception as exc:
    ...
    def engage_killswitch(self) -> None:
        """Global emergency stop — tasks, goals, TTS, in-flight queries."""
