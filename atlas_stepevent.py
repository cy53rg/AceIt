...
_STEP_TIMEOUT_DEFAULT = 12.0
STEP_TIMEOUT_S = max(
    1.0,
    float(os.environ.get("ATLAS_STEP_TIMEOUT_S") or _STEP_TIMEOUT_DEFAULT),
)


class StepOrchestratorStalled(Exception):
    """Raised when the UI step handler stops responding to consecutive steps."""
    ...
    def run_step(
        ...
        if not pending.done.wait(timeout=timeout_s):
            ...
            if self._consecutive_timeouts >= 2:
                raise StepOrchestratorStalled(
                    "UI step handler stopped responding after "
